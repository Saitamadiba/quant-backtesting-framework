...
sel_strategy = st.sidebar.selectbox(
    ...
    help="Select the live bot strategy to compare against its WFO adapter reconstruction.",
)
...
sel_symbol = st.sidebar.selectbox(
    ...
    help="Asset to analyze. Shadow comparison runs on this symbol only.",
)
...
m1.metric("Match Rate", f"{match_rate:.1%}",
          ...
          help="Percentage of live trades that had a matching shadow backtest trade (same timing and direction). >20% = good alignment between live bot and adapter.")
m2.metric("Live Trades", data.get("n_live_trades", 0),
          help="Total signals/trades executed by the live bot during the comparison period.")
m3.metric("Shadow Trades", data.get("n_shadow_trades", 0),
          help="Total signals generated by the WFO adapter reconstruction over the same period.")
m4.metric("Matched", data.get("n_matched", 0),
          help="Trades appearing in both live and shadow (same timing and direction). These validate the adapter faithfully reproduces the bot's logic.")
m5.metric("Phantom", data.get("n_phantom", 0),
          help="Trades only in shadow, not in live bot. May indicate the live bot has additional filters, or market conditions differed.")

# ── Signal Timeline ───────────────────────────────────────────────────────────
st.markdown("---")
st.subheader("Signal Timeline")
st.caption("Green markers = matched trades (both live and shadow agree). Red = live-only (adapter missed). Orange = shadow-only (phantoms the live bot didn't take).")
...
st.caption("Breakdowns by direction (LONG/SHORT) and time of day reveal systematic differences between the live bot and the adapter reconstruction.")
...
if agg:
    st.markdown("---")
    st.caption("Summary statistics including average time difference between matched signals and directional concordance rates.")
